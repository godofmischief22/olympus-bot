...
class Customrole(commands.Cog):
    ...
    async def reset(self, context: Context) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            ...
            description=f"Deleted All Custom Role commands <:olympus_tick:1368119361440845824>\n\n**Removed Roles:**\n" + "\n".join(removed_roles) if removed_roles else "No roles were previously set.",
            color=0x000000
        )
    ...
    async def on_message(self, message: discord.Message):
        ...
        if result:
            ...
            if reqrole is None:
                await message.channel.send("<a:Warning:1368436682495365171> The required role is not set up in this server. Please set it up using `setup reqrole`.")
                return

            
            if reqrole not in message.author.roles:
                await message.channel.send(embed=discord.Embed(description=f"<a:Warning:1368436682495365171> You need the {reqrole.mention} role to use this command.", color=0x000000))
                return
            ...
            try:
                if role in member.roles:
                    ...
                    await message.channel.send(embed=discord.Embed(
                        title="<:olympus_tick:1368119361440845824> Success",
