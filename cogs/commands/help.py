...
class HelpCommand(commands.HelpCommand):
    ...
    async def on_help_command_error(self, ctx, error):
        ...
        if not isinstance(error, tuple(errors)):
            await self.context.reply(f"Unknown Error Occurred\n{getattr(error, 'original', error)}", mention_author=False)
    ...
    async def send_bot_help(self, mapping):
        ...
        total_cmds = len(set(self.context.bot.walk_commands()))
...
```\n{sonu}""",
        ...
        embed.add_field(name="**Aliases**", value=alias if alias else "No Aliases", inline=False)
        embed.add_field(name="**Usage**", value=f"`{self.context.prefix}{command.signature}`")
        embed.set_author(name=f"{command.qualified_name.title()} Command", icon_url=self.context.bot.user.display_avatar.url)
    ...
    def get_command_signature(self, command: commands.Command) -> str:
        ...
        if command.aliases:
            ...
            alias = command.name
        return f'{parent + " " if parent else ""}{alias} {command.signature}'
    ...
    async def send_group_help(self, group):
        ...
        entries = [
            (f"`{self.context.prefix}{cmd.qualified_name}`", f"{cmd.short_doc or ''}\n\u200b")
